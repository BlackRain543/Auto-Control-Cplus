...
const double ref_freq = 2000.0;   // 参考频率
const double lock_band = 1.0;      // 锁定偏差
const double kp = 0.8;              // PI控制器的比例系数
const double ki = 0.1;              // PI控制器的积分系数

double input_signal = 300;    // 输入信号
double ref_signal = 1000;   // 参考信号
...
//    phase_error = std::arg(input_signal) - std::arg(ref_signal);
    phase_error = input_signal - ref_signal;
